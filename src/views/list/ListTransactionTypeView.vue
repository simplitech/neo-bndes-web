  ...
  <div class="verti">
    <section class="header">
      <div class="horiz items-center gutter-10">
        ...
        <router-link to="/transaction-type/new" class="btn primary">
          {{ $t('app.add') }}
        </router-link>
      </div>
    </section>
            ...
            <table>
              ...
              <tr v-for="(data, i) in collection.data" :key="i">
                <td class="horiz nowrap">
                  <a @click="pushByName('editTransactionType', collection.items[i].$id)" class="icon icon-pencil"></a>
                </td>
...
<script lang="ts">
  ...
  export default class ListTransactionTypeView extends Mixins<MixinQueryRouter>() {
    collection = new PagedResp(TransactionType)
    pushByName = pushByName

    async downloadCsv() {
      const csv = new PagedResp(TransactionType, {}, null, null)
